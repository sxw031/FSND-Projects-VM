...
# list of players are added to the Database TABLE
the_players = [
	(1, 'Jeff'),
	(2, 'Adarsh'),
	(3, 'Amanda'),
...
def registerPlayerUpdated(player_id, name):
	""" Add a player to the tournament database.
		...
		The database assigns a unique serial id number for the player. This should be handled by your SQL database schema, not in your Python code.
		
		Args:
			name: the player's full name (need not to be unique).
	"""
	db = connect()
	db_cursor = db.cursor()
...
def createRandomMatches(player_list, num_matches):
	"""
		function to create some random matches
	"""
